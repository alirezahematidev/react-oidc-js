...
export class User {
  /**
   * A JSON Web Token (JWT). Only provided if `openid` scope was requested. The
   * application can access the data decoded by using the `profile` property.
   */
  ...
  /**
   * The requested access token returned from the OIDC provider. The application
   * can use this token to authenticate itself to the secured resource.
   */
  public access_token: string;

  /**
   * An OAuth 2.0 refresh token. The app can use this token to acquire
   * additional access tokens after the current access token expires. Refresh
   * tokens are long-lived and can be used to maintain access to resources for
